...
type roDataStats struct {
	...
	SelfRel    int `json:"self_relative"`
...
func reportRoData(st roDataStats) {
	fmt.Fprintf(os.Stderr, "rodata switch tables: %d of %d spans applied (%d self-relative, %d rebased), %d entries, %d retargeted, %d unresolved, %d unplaced\n",
		st.Tables, st.Candidates, st.SelfRel, st.Rebased, st.Entries, st.Retargeted, st.Unresolved, st.Unplaced)
}
